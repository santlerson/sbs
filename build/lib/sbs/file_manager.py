...
class FileManager:
        ...
        if do:
            try:
                ...
                media_body = MediaIoBaseUpload(
                    BytesIO(
                        self.c.encrypt(json_data.encode('UTF-8'))
                    ),
                ...
                file = self.service.files().create(body=metadata, media_body=media_body, fields="id")
                response = None
                bar = tqdm(total=1, dynamic_ncols=True)
                total = 0.0
                while response is None:
                    try:
                        status, response = file.next_chunk()
    ...
    def migrate_to_app_data(self):
        print(self.service.files().update(fileId=self.config.parent_id, addParents=['appDataFolder']).execute())
        print("success")
    def list_backups(self):

        files = self.service.files().list(q="'{}' in parents".format(self.config.parent_id)).execute()['files']
